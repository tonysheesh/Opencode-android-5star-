...
# opencode.json, and 6 subagent files in .opencode/agent/ — all from
# scratch, no downloads, no zip extraction, no external dependencies.
#
# Usage:
#   Run this FROM your Android project root (where build.gradle.kts lives),
#   or pass a target directory as the first argument:
#     bash setup-opencode-android.sh /path/to/your-android-project
#
set -euo pipefail
...
if [ "$MISSING" -eq 0 ]; then
  ...
  echo ""
  echo "Next steps:"
  echo "  1. cd \"$TARGET_DIR\" if you're not already there"
  echo "  2. Run: opencode"
  echo "  3. Talk to the lead agent: @android-lead <your feature request>"
else
